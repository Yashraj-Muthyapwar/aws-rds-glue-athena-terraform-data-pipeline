...
echo "export TF_VAR_source_username="$DB_USERNAME"" >> $HOME/.bashrc
echo "export TF_VAR_source_password="$DB_PASSWORD"" >> $HOME/.bashrc
...
aws s3 cp ./terraform/assets/glue_job/etl_job.py s3://$TF_VAR_scripts_bucket/etl_job.py
echo "Glue script has been uploaded to S3"

# Inject Terraform state file path into backend.tf
